...
## som
mmpi <- rename_all(mmpi,list(~str_replace(.,"UWS","MLS")))
mmpi <- rename_all(mmpi,list(~str_replace(.,"MAG","GIC")))
mmpi <- rename_all(mmpi,list(~str_replace(.,"KOP","HPC")))
mmpi <- rename_all(mmpi,list(~str_replace(.,"NRO","NUC")))
mmpi <- rename_all(mmpi,list(~str_replace(.,"KOG","COG")))
...
## intern
## mmpi <- rename_all(mmpi,list(~str_replace(.,"","SUI")))
## mmpi <- rename_all(mmpi,list(~str_replace(.,"","HLP")))
...
## externl
mmpi <- rename_all(mmpi,list(~str_replace(.,"JVP","JCP")))
## mmpi <- rename_all(mmpi,list(~str_replace(.,"","SUB")))
## mmpi <- rename_all(mmpi,list(~str_replace(.,"","AGG")))
mmpi <- rename_all(mmpi,list(~str_replace(.,"AKT","ACT")))

## interp
## mmpi <- rename_all(mmpi,list(~str_replace(.,"","FML")))
## mmpi <- rename_all(mmpi,list(~str_replace(.,"","IPP")))
mmpi <- rename_all(mmpi,list(~str_replace(.,"SMV","SAV")))
## mmpi <- rename_all(mmpi,list(~str_replace(.,"","SHY")))
mmpi <- rename_all(mmpi,list(~str_replace(.,"MIS","DSF")))
...
res <- sapply(
    ...
    function(X){
      d <- t(mmpi[X])
      ret <- switch(which(mmpi.dims==X), "Name" = d ,
                    "VRIN-r"
                    =
                    sapply(d,function(X){
                   ...
                   ,
                    ...
                    sapply(d,function(X){
                      if ( X >= 80 ) {
                        "Client reports a generalized sense of physical well-being."
                      } else if ( X >= 65 ) {
                        "Client reports experiencing poor health and feeling weak or tired. "
                      } else if ( X >= 39 ) {
                        ""
                      } else  {
                        "Client reports a generalized sense of physical well-being."
                   ...
                   ,
                    ...
                    sapply(d,function(X){
                      if ( X >= 90 ) {
                        "Client reports a large number of gastrointestinal complaints such as poor appetite, nausea, vomiting, and recurrent upset stomach."
                      } else if ( X >= 65 ) {
                        "Client reports a number of gastrointestinal complaints."
                      } else  {
                        ""
                      } 
                    })
                   ,
                    ...
                    sapply(d,function(X){
                      if ( X >= 80 ) {
                        "Client reports diffuse head and neck pain, recurring headaches, and development of head pain when upset."
                      } else if ( X >= 65 ) {
                        "Client reports head pain."
                      } else  {
                        ""
                      } 
                    })
                   ,
                    ...
                    sapply(d,function(X){
                      if ( X >= 92 ) {
                        "Client reports vague neurological complaints."
                      } else if ( X >= 65 ) {
                        "Client reports a large number of various neurological complaints (e.g., dizziness, loss of balance, numbness, weakness, paralysis, loss of control over movement including involuntary movements)."
                      } else {
                        ""
                      }
                    })
                   ,
                    ...
                    sapply(d,function(X){
                      if ( X >= 81 ) {
                        "Client reports a pattern of cognitive difficulties including memory problems, difficulties concentrating, intellectual limitations, and confusion."
                      } else if ( X >= 65 ) {
                        "Client reports a diffuse pattern of cognitive difficulties."
                      } else {
                        ""
                      }
                    })
                   ...
                   ,
                    ...
                    sapply(d,function(X){
                      if ( X >= 70 ) {
                        "Client reports feeling useless and lacking confidence."
                      } else if ( X >= 65 ) {
                        "Client reports feelings of self-doubt."
                      } else {
                        ""
                      }
                    })
                   ...
                   ,
                    ...
                    sapply(d,function(X){
                      if ( X >= 78 ) {
                        "Client reports multiple specific fears such as blood, fire, thunder, water, natural disasters, spiders, mice, and other animals."
                      } else if ( X >= 65 ) {
                        "Client reports multiple specific fears of certain animals and acts of nature."
                      } else if (X ) {
                        ""
                      }
                    })
                   ...
                   ,
                    ...
                    sapply(d,function(X){
                      if ( X >= 80 ) {
                        "Client reports a significant history of substance abuse, current substance abuse, frequent use of alcohol and drugs, and/or using alcohol to “relax and open up."
                      } else if ( X >= 65 ) {
                        "Client reports significant past and current substance abuse."
                      } else if ( X >= 39) {
                        ""
                      } else {
                        "Client reports a lower-than-average number of specific fears."
                      }
                    })
                   ,
                   ...
                   ,
                    ...
                    sapply(d,function(X){
                      if ( X < 39 ) {
                        "The client shows no interest in aesthetic﻿–literary activities. "
                   ...
                   ,
                    ...
                    sapply(d,function(X){
                      if ( X < 39 ) {
                        "The client shows no interest in mechanical-physical activities. "
                      } else {
                        ""
                      }
                    })
